...
+ (void)requestGETDataWithPath:(NSString *)path
    ...
    NSLog(@"requestURL = %@, paramters = %@",requestURL,paramters);
    ...
    if (xCache.uuid) {
        [headerFieldValueDictionary setObject:xCache.uuid forKey:@"uuid"];
    } else {
        [headerFieldValueDictionary setObject:@"2574b49e57334348ab576b7540c332e1" forKey:@"uuid"];
    }
    ...
    [NetWorkingManager shareManager].requestSerializer = requestSerializer;
    NSLog(@"token = %@, uuid = %@",headerFieldValueDictionary[@"token"],headerFieldValueDictionary[@"uuid"]);
    ...
    [[NetWorkingManager shareManager] GET:path parameters:paramters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (failure) {
            [SVProgressHUD showErrorWithStatus:@"Network wrong"];
            failure(error);
            NSLog(@"%@ 服务器错误",requestURL);
        }
    }];
...
+ (void)sendPOSTDataWithPath:(NSString *)path
    ...
    [[NetWorkingManager shareManager] POST:requestURL parameters:paramters progress:^(NSProgress * _Nonnull uploadProgress) {
...
+ (void)sendPOSTImageWithPath:(NSString *)path
    ...
    [[NetWorkingManager shareManager] POST:requestURL parameters:paramters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
//        @try{
//            
//            imageFilePath = [path stringByAppendingPathComponent:IMGNAME];
//            [imgData writeToFile:imageFilePath atomically:true];
//            
//        } @catch (NSException *exception) {
//            NSLog(@"上传出错");
//        }
        // 拼接Data
        [formData appendPartWithFileData:imgData name:@"avatar" fileName:@"user_avatar.png" mimeType:@"image/jpeg"];
...
+ (void)sendPOSTImageWithPath:(NSString *)path
    ...
    [[NetWorkingManager shareManager] POST:requestURL parameters:paramters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (UIImage *image in imageArray) {
            ...
            NSData *imgData = UIImageJPEGRepresentation(resizedImage, 0.5);
            // 拼接Data
            [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"image[]"] fileName:@"image.png" mimeType:@"image/jpeg"];
            i++;
        }
 ...
 *  @param failure        请求失败的回调
 */
+ (void)sendPOSTImageMoreWithPath:(NSString *)path
                withParamters:(NSDictionary *)paramters
               withImageArray:(NSArray *)imageArray
              withtargetWidth:(CGFloat )width
                 withProgress:(void(^) (float progress))upLoadProgress
    ...
    [[NetWorkingManager shareManager] POST:requestURL parameters:paramters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (int i = 0; i < imageArray.count; i++) {
            // 压缩图片，指定宽度
            UIImage *resizedImage = [UIImage imageCompressed:imageArray[i] withdefineWidth:width];
            NSData *imgData = UIImageJPEGRepresentation(resizedImage, 0.5);
            // 拼接Data
            if (i == 0) {
                [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"image"] fileName:@"image.png" mimeType:@"image/jpeg"];
            } else {
                [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"bgcImage"] fileName:@"image.png" mimeType:@"image/jpeg"];
            }
            index++;
        }
